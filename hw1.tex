...
In terms of accuracy, the batch gradient descent was generally more accurate than the stochastic gradient descent. However, in terms of the number of evaluations, the stochastic required fewer iterations per parameter update. This was because in each iteration of batch gradient descent, $n$ evaluations were necessary, compared with stochastic gradient descent which evaluated the a single point-wise gradient per update iteration. Overall however, we found that batch gradient descent required fewer overall evaluations than stochastic gradient descent.
